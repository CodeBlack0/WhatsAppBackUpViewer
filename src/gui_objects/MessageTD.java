...
//		}
                ...
                Label lbl;
                if (actor != null) lbl = new Label(String.format("Am: %s | von: %s%n%s", dateStr, actor, att));
                else  lbl = new Label(String.format("Am: %s%n%s", dateStr, att));
...
//                  }
                ...
                node.setOnMouseEntered(t->{ if (cursorSetter != null) cursorSetter.setCursor(Cursor.HAND);});
                node.setOnMouseExited(t->{ if (cursorSetter != null)  cursorSetter.setCursor(Cursor.DEFAULT);});
...
//		backgrRct.setHeight( txtArea.getBoundsInLocal().getHeight() + 2d*insets);
                ...
                String dateStr = getStringDate(message.get_timestamp());
                Text lbl;
                if (actor != null) lbl = new Text(String.format("Am: %s | von: %s", dateStr,  actor));
                else lbl = new Text(String.format("Am: %s", dateStr));
                ...
                Line line = new Line(0, yOffs, backgrRct.getWidth(), yOffs);
