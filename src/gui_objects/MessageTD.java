...
//		}
	...
	private File getAttachmentFile(File file){
		File tarFile = file;
	...
	private void addChilds(double width){
		if (message != null){
			if (message.get_content().endsWith("3955.jpp")){
...
//	//			}
				...
				if (att.endsWith(".jpg") || att.endsWith(".png")){
					...
					File tarFile = getAttachmentFile(file);
	...
	//					imgVw.setScaleY(scaleFactor);
				...
				node.setOnMouseClicked(t->{
					if (file != null){
						final File tarFile = getAttachmentFile(file);
