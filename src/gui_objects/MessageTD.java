...
//					imgVw.setScaleY(scaleFactor);
             ...
             // Windows {
                ...
                node.setOnMouseEntered(t->{
                    if (cursorSetter != null) cursorSetter.setCursor(Cursor.HAND);
                });
                node.setOnMouseExited(t->{
                    if (cursorSetter != null) cursorSetter.setCursor(Cursor.DEFAULT);
                });
...
//			backgrRct.setHeight( txtArea.getBoundsInLocal().getHeight() + 2d*insets);
                ...
                Text lbl;
                if (actor != null)
                    lbl = new Text(String.format("Am: %s | von: %s", dateStr, actor));
                else{
