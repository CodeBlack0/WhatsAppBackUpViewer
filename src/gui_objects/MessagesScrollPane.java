...
public class MessagesScrollPane extends Pane {
    ...
    public MessagesScrollPane(List<Message> messages, final Scene scene, final ScrBarValueSetter scrBarValSetter){
        ...
        this.setOnScroll(t->{ scrBarValSetter.setValueOffset( t.getDeltaY() > 0 ? -1 : 1 ); });
//      key up && down funktioniert noch nicht, da die toolbar staendig den fokus fuer sich beansrpucht...
//	this.setOnKeyPressed(t->{
//          if (t.getCode() == KeyCode.UP) scroll(-1);
//          else if (t.getCode() == KeyCode.DOWN) scroll(1);
//      });
        ...
        this.widthProperty().addListener(t->{ properBackgroundID(); });
        this.heightProperty().addListener(t->{ properBackgroundID();  });
        this.getChildren().addAll(backgrImgVw, scrPneGrp);

        this.heightProperty().addListener(t->{ renderMessages(); });
        this.widthProperty().addListener(t->{ renderMessages(); });
